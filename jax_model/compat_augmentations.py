...
from albumentations import Compose as BaseCompose
...
class OneOf(BasicTransform):
    ...
    def __init__(self, transforms, 
        ...
        s = sum(transforms_ps)
        if s <= 1e-6:
            self.transforms_ps = [1.0 / len(transforms_ps)] * len(transforms_ps)
        else:
...
class PoseDrop2(BasicTransform):
    ...
    def __init__(
        ...
        # Fix for odd number of landmarks (33)
        if len(pose_indices) % 2 != 0:
            pose_indices = pose_indices[:len(pose_indices)//2 * 2]
